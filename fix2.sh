...
# Force delete both problem files first
...
ALLOWEOF
echo "✓ AllowListInterceptor.kt written ($(wc -l < $BASE/network/AllowListInterceptor.kt) lines)"
...
MAINEOF
echo "✓ MainActivity.kt written ($(wc -l < $BASE/ui/MainActivity.kt) lines)"

# Verify both files look clean
echo ""
echo "Verifying no duplicate class declarations..."
grep -c "class AllowListInterceptor" $BASE/network/AllowListInterceptor.kt
grep -c "class MainActivity" $BASE/ui/MainActivity.kt
echo "Both should show 1"

echo ""
echo "Done. Now run:"
echo "  git add -A && git commit -m 'fix AllowListInterceptor and MainActivity' && git push"
